      ...
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          _buildSection('Product Information', [
            ...
            'Warranty Status: ${ticket.warrantyStatus.toUpperCase()}',
          ]),
          _buildSection('Service Details', [
            'Ticket ID: ${ticket.id}',
            'Issue: ${ticket.issueDescription}',
            'Received On: ${ticket.issueReceivedDate.toString().split(' ')[0]}',
            'SLA Commit: ${ticket.serviceSLAReplyDays} Days',
          ]),
          if (ticket.usedParts.isNotEmpty)
            _buildPartsTable(ticket.usedParts),
          pw.SizedBox(height: 10),
          _buildSection('Work Summary', [
            ticket.finalServiceSummary ?? 'No summary provided.',
          ]),
           pw.SizedBox(height: 10),
          _buildSection('Full Notes History', [
            ticket.mergedNotesSummary ?? 'No notes available.',
          ]),
          pw.SizedBox(height: 20),
          if (ticket.feedbackText != null)
             _buildSection('Customer Feedback', [
               'Rating: ${ticket.rating}/5',
               'Comment: ${ticket.feedbackText}',
             ]),
          pw.Divider(),
          _buildFooter(),
        ],
